import{n as e,r as t,t as n}from"./auth-3JZ8j-Zd.js";async function r(t){let{data:n,error:r}=await e.from(`waste_submissions`).select(`*`).eq(`status`,t).order(`created_at`,{ascending:!1});if(r)throw r;return n||[]}async function i(t,n){let{data:r,error:i}=await e.from(`waste_submissions`).update({status:n}).eq(`id`,t);if(i)throw i;return r}async function a(){let{data:t,error:n}=await e.from(`waste_groups`).select(`id, name_vi`).order(`sort_order`);if(n)throw n;return t||[]}async function o({slug:t,nameVi:n,funFact:r,imageUrl:i,groupId:a,userId:o}){let{error:s}=await e.from(`waste_dictionary`).insert({slug:t,name_vi:n,fun_fact:r||null,image_url:i||null,waste_group_id:parseInt(a),created_by:o,is_active:!0});if(s)throw s}async function s(t,n,r){let{data:i,error:a}=await e.rpc(`admin_get_users`,{p_page:t,p_limit:n,p_search:r||null});if(a)throw a;return i||[]}async function c(t,n,r){let{error:i}=await e.rpc(`admin_ban_user`,{p_user_id:t,p_reason:r?n:null,p_is_locked:r});if(i)throw i}async function l(t){let{error:n}=await e.auth.resetPasswordForEmail(t);if(n)throw n}async function u(){let{data:t,error:n}=await e.from(`profiles`).select(`id, display_name`);if(n)throw n;return t||[]}async function d(t,n,r){let i=e.from(`privileged_action_requests`).select(`*`,{count:`exact`});t&&(i=i.eq(`state`,t));let{data:a,count:o,error:s}=await i.order(`created_at`,{ascending:!1}).range(n,r);if(s)throw s;return{data:a||[],count:o}}async function ee(t){let{data:n,error:r}=await e.from(`privileged_action_approvals`).select(`*`).eq(`request_id`,t);if(r)throw r;return n||[]}async function f(t,n){let{data:r,error:i}=await e.rpc(`privileged_action_add_approval`,{p_request_id:t,p_comment:n||null});if(i)throw i;return r}async function p(t,n){let{error:r}=await e.from(`privileged_action_requests`).update({state:`rejected`,execution_note:n}).eq(`id`,t);if(r)throw r}async function m({actionType:t,payload:n,requesterId:r,reason:i}){let{error:a}=await e.from(`privileged_action_requests`).insert({action_type:t,payload:n,requester_id:r,execution_note:i});if(a)throw a}async function h(t,n){if(t===`promote_user_admin`){let{error:t}=await e.from(`profiles`).update({role:`admin`}).eq(`id`,n.user_id);if(t)throw t}else if(t===`demote_admin`){let{error:t}=await e.from(`profiles`).update({role:`user`}).eq(`id`,n.user_id);if(t)throw t}else if(t===`delete_collection_point`){let{error:t}=await e.from(`collection_points`).delete().eq(`id`,n.point_id);if(t)throw t}else if(t===`toggle_kill_switch`){let{data:t}=await e.from(`system_settings`).select(`value`).eq(`key`,`maintenance`).maybeSingle(),r=t?.value||{enabled:!1,message:``};r.kill_switch=n.kill_switch;let{error:i}=await e.from(`system_settings`).update({value:r}).eq(`key`,`maintenance`);if(i)throw i}else throw Error(`Chưa hỗ trợ thực thi tự động cho loại hành động này.`)}async function g(t,n){let{error:r}=await e.from(`privileged_action_requests`).update({state:`executed`,executed_at:new Date().toISOString(),execution_note:n}).eq(`id`,t);if(r)throw r}async function _(){let{data:t,error:n}=await e.from(`system_settings`).select(`*`);if(n)throw n;return t||[]}async function v({maintenanceVal:t,pointsVal:n,geminiVal:r,userId:i}){let{error:a}=await e.from(`system_settings`).update({value:t,updated_by:i}).eq(`key`,`maintenance`),{error:o}=await e.from(`system_settings`).update({value:n,updated_by:i}).eq(`key`,`points`),{error:s}=await e.from(`system_settings`).update({value:r,updated_by:i}).eq(`key`,`gemini`);if(a||o||s)throw Error(`Lỗi cập nhật cấu hình: `+(a?.message||o?.message||s?.message))}async function y(t,n,r){let i=e.from(`collection_points`).select(`*`,{count:`exact`});t!=null&&t!==``&&(i=i.eq(`is_verified`,t===`true`||t===!0));let{data:a,count:o,error:s}=await i.order(`created_at`,{ascending:!1}).range(n,r);if(s)throw s;return{data:a||[],count:o}}async function b(t){let{error:n}=await e.from(`collection_points`).update({is_verified:!0}).eq(`id`,t);if(n)throw n}async function x(t){let{error:n}=await e.from(`collection_points`).delete().eq(`id`,t);if(n)throw n}async function S(t,n,r){let i=e.from(`game_questions`).select(`
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="bg-slate-100 dark:bg-slate-950 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 flex items-center justify-center min-h-[180px]">
                ${t.scan_image_path?`<img src="${z(t.scan_image_path)}" class="w-full h-full object-contain">`:`<p class="p-10 text-center text-slate-400 dark:text-slate-600 font-semibold text-xs uppercase tracking-wider">KHÔNG CÓ ẢNH</p>`}
            </div>
            <div class="space-y-3">
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    ...
                    <p class="text-lg font-bold text-slate-900 dark:text-white mt-1 font-heading">${z(t.suggested_name_vi||`N/A`)}</p>
                </div>
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nhãn AI (TFLite)</span>
                    <p class="font-bold text-emerald-600 dark:text-emerald-400 mt-1">${z(t.tflite_top_label||`N/A`)} (${(t.tflite_confidence*100).toFixed(1)}%)</p>
                </div>
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Trạng thái</span>
                    <p class="font-bold text-emerald-600 dark:text-emerald-400 uppercase mt-1 text-sm">${z(t.status)}</p>
                </div>
            </div>
        </div>
        <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Phân tích Gemini</span>
            <p class="text-slate-700 dark:text-slate-300 text-sm mt-1.5 leading-relaxed">${z(t.gemini_payload?.result_text||`Chưa có phân tích`)}</p>
        </div>
        <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <p class="text-slate-700 dark:text-slate-300 text-sm mt-1.5">${z(t.suggested_fun_fact||`N/A`)}</p>
        </div>
        ${t.status===`pending_review`?`
            <div class="pt-4 border-t border-slate-200 dark:border-slate-800 flex gap-3">
                <button onclick="approveWithData('${z(t.id)}')" class="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-xl font-bold text-sm transition shadow-sm">DUYỆT VÀO HỆ THỐNG</button>
                <button onclick="updateStatus('${z(t.id)}', 'rejected')" class="flex-1 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 py-3 rounded-xl font-bold text-sm transition">TỪ CHỐI</button>
            </div>
        `:``}
    `,document.getElementById(`detail-modal`).classList.remove(`hidden`),document.getElementById(`detail-modal`).classList.add(`flex`)},window.closeDetailModal=()=>{document.getElementById(`detail-modal`).classList.add(`hidden`),document.getElementById(`detail-modal`).classList.remove(`flex`)},window.fetchWasteGroups=async()=>{try{k=await a();let e=document.getElementById(`approve-group`);e&&(e.innerHTML=`<option value="">-- Chọn nhóm rác --</option>`,k.forEach(t=>{e.innerHTML+=`<option value="${t.id}">${t.name_vi}</option>`}));let t=document.getElementById(`question-group`);t&&(t.innerHTML=`<option value="">-- Chọn nhóm phân loại đúng --</option>`,k.forEach(e=>{t.innerHTML+=`<option value="${e.id}">${e.name_vi}</option>`}));let n=document.getElementById(`filter-q-group`);n&&(n.innerHTML=`<option value="">Tất cả Nhóm rác</option>`,k.forEach(e=>{n.innerHTML+=`<option value="${e.id}">${e.name_vi}</option>`}))}catch(e){console.error(`Lỗi tải nhóm rác:`,e)}},window.approveWithData=e=>{let t=O.find(t=>t.id===e);t&&(document.getElementById(`approve-id`).value=t.id,document.getElementById(`approve-image-url`).value=t.scan_image_path||``,document.getElementById(`approve-name`).value=t.suggested_name_vi||``,document.getElementById(`approve-funfact`).value=t.suggested_fun_fact||``,document.getElementById(`approve-group`).value=``,document.getElementById(`approve-modal`).classList.remove(`hidden`),document.getElementById(`approve-modal`).classList.add(`flex`))},window.closeApproveModal=()=>{document.getElementById(`approve-modal`).classList.add(`hidden`),document.getElementById(`approve-modal`).classList.remove(`flex`)},window.submitApproveData=async()=>{let t=document.getElementById(`btn-submit-approve`),n=document.getElementById(`approve-id`).value,r=document.getElementById(`approve-image-url`).value,a=document.getElementById(`approve-name`).value.trim(),s=document.getElementById(`approve-group`).value,c=document.getElementById(`approve-funfact`).value.trim();if(!a||!s)return alert(`Vui lòng nhập tên và chọn nhóm rác!`);let l=R(a);l+=`-`+B(4),t.disabled=!0,t.innerText=`ĐANG LƯU...`;try{let{data:{session:t}}=await e.auth.getSession(),u=t?.user?.id;await o({slug:l,nameVi:a,funFact:c,imageUrl:r,groupId:s,userId:u}),await i(n,`approved`),alert(`Đã duyệt và lưu vào từ điển thành công!`),closeApproveModal(),closeDetailModal(),window.fetchSubmissions()}catch(e){console.error(e),alert(e.message)}finally{t.disabled=!1,t.innerText=`XÁC NHẬN LƯU VÀO TỪ ĐIỂN`}},window.updateStatus=async(e,t)=>{try{await i(e,t),closeDetailModal(),window.fetchSubmissions()}catch(e){alert(e.message)}},window.switchTab=e=>{document.querySelectorAll(`.tab-content`).forEach(e=>e.classList.add(`hidden`)),document.querySelectorAll(`nav button`).forEach(e=>e.classList.remove(`sidebar-active`,`hover:bg-slate-100`,`dark:hover:bg-slate-800`));let t=document.getElementById(`tab-`+e),n=document.getElementById(`btn-`+e);t&&t.classList.remove(`hidden`),n&&n.classList.add(`sidebar-active`),document.querySelectorAll(`nav button`).forEach(e=>{e.classList.contains(`sidebar-active`)||e.classList.add(`hover:bg-slate-100`,`dark:hover:bg-slate-800`)}),window.toggleMobileSidebar(!1),e===`submissions`&&window.fetchSubmissions(),e===`users`&&window.fetchUsers(1),e===`privileged_actions`&&window.fetchPrivilegedActions(1),e===`settings`&&window.fetchSystemSettings(),e===`collection_points`&&window.fetchCollectionPoints(1),e===`game_questions`&&window.fetchGameQuestions(1)};function V(e){let t=document.getElementById(`btn-user-prev`),n=document.getElementById(`btn-user-next`),r=document.getElementById(`user-page-info`),i=(A-1)*j+1,a=Math.min(A*j,e);e===0?(r.innerText=`Không có dữ liệu`,t.disabled=!0,n.disabled=!0):(r.innerText=`Hiển thị ${i}-${a} trên ${e}`,t.disabled=A===1,n.disabled=a>=e)}window.fetchUsers=async(e=1)=>{A=e;let t=document.getElementById(`table-users`),n=document.getElementById(`loader-users`),r=document.getElementById(`filter-user-search`).value,i=document.getElementById(`filter-user-role`).value,a=document.getElementById(`filter-user-status`).value;t.innerHTML=``,n.classList.remove(`hidden`),document.getElementById(`user-page-info`).innerText=`Đang tải...`;try{let e=await s(A,j,r);n.classList.add(`hidden`);let o=e||[];if(i&&(o=o.filter(e=>e.role===i)),a){let e=a===`locked`;o=o.filter(t=>t.is_locked===e)}if(o.length===0){t.innerHTML=`<tr><td colspan="5" class="p-8 text-center text-slate-400 dark:text-slate-500 font-semibold text-sm">Không tìm thấy người dùng nào</td></tr>`,V(0);return}let c=e.length>0&&e[0].total_count?parseInt(e[0].total_count):o.length;o.forEach(e=>{let n=document.createElement(`tr`);n.className=`hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition border-b border-slate-200 dark:border-slate-800/60 last:border-none`;let r=e.role===`super_admin`?`bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-500/10 dark:border-purple-500/30 dark:text-purple-400`:e.role===`admin`?`bg-sky-50 text-sky-700 border-sky-200 dark:bg-cyan-500/10 dark:border-cyan-500/30 dark:text-cyan-400`:`bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400`,i=e.is_locked?`<span class="px-2.5 py-0.5 rounded-full bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400 font-bold text-[11px] uppercase tracking-wider flex items-center justify-center gap-1.5 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-rose-500"></div> Bị khóa</span>`:`<span class="px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-bold text-[11px] uppercase tracking-wider flex items-center justify-center gap-1.5 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> Hoạt động</span>`,a=e.role===`super_admin`&&M!==`super_admin`;n.innerHTML=`
                <td class="p-3.5 sm:p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <p class="font-semibold text-slate-900 dark:text-white text-sm truncate font-heading">${z(e.display_name||`Chưa cập nhật`)}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400 font-normal truncate">${z(e.email||`Ẩn email`)}</p>
                        </div>
                    </div>
                </td>
            ...
            `,t.appendChild(n)}),H(s||0)}catch(e){n.classList.add(`hidden`),t.innerHTML=`<tr><td colspan="5" class="p-8 text-center text-rose-500 font-semibold text-sm">Lỗi: ${z(e.message)}</td></tr>`}},window.changePAPage=e=>{window.fetchPrivilegedActions(P+e)},window.showPADetail=async t=>{let n=N.find(e=>e.id===t);if(!n)return;let r=document.getElementById(`pa-detail-modal`),i=document.getElementById(`pa-detail-content`);i.innerHTML=`<p class="text-slate-500 dark:text-slate-400 font-medium text-center text-sm">Đang tải chi tiết phê duyệt...</p>`,r.classList.remove(`hidden`);try{let r=await ee(t),a=await u(),o={};a.forEach(e=>o[e.id]=e.display_name||e.id);let{data:{session:s}}=await e.auth.getSession(),c=s?.user?.id,l=I[n.action_type]||n.action_type,d=L[n.state]||n.state,f=o[n.requester_id]||n.requester_id,p=r&&r.some(e=>e.approver_id===c),m=n.requester_id===c,h=``;h=r&&r.length>0?r.map(e=>`
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="space-y-3">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                        ...
                        <p class="font-bold text-emerald-600 dark:text-emerald-400 mt-1">${f}</p>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                        <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Trạng thái</span>
        ...
        `}catch(e){i.innerHTML=`<div class="p-8 text-rose-500 font-semibold text-sm text-center">Lỗi tải chi tiết: ${z(e.message)}</div>`}},window.closePADetailModal=()=>{document.getElementById(`pa-detail-modal`).classList.add(`hidden`)},window.approvePARequest=async e=>{let t=prompt(`Nhập ý kiến phê duyệt (Không bắt buộc):`);if(t!==null)try{let n=await f(e,t);alert(`Đã phê duyệt thành công! Trạng thái hiện tại: `+(I[n]||n)),closePADetailModal(),window.fetchPrivilegedActions(P)}catch(e){alert(`Lỗi phê duyệt: `+e.message)}},window.rejectPARequest=async t=>{let n=prompt(`Nhập lý do từ chối (Bắt buộc):`);if(n)try{let{data:{session:r}}=await e.auth.getSession();await p(t,`Bị từ chối bởi ${r?.user?.email||`Admin`}. Lý do: ${n}`),alert(`Đã từ chối yêu cầu này!`),closePADetailModal(),window.fetchPrivilegedActions(P)}catch(e){alert(`Lỗi: `+e.message)}},window.executePARequest=async t=>{let n=N.find(e=>e.id===t);if(n&&confirm(`Bạn chắc chắn muốn thực thi hành động này trực tiếp lên hệ thống?`))try{let{data:{session:r}}=await e.auth.getSession(),i=r?.user?.email||`Admin`;await h(n.action_type,n.payload),await g(t,`Thực thi thành công bởi ${i}`),alert(`Thực thi hành động thành công!`),closePADetailModal(),window.fetchPrivilegedActions(P)}catch(e){alert(`Lỗi thực thi: `+e.message)}},window.openCreatePAModal=async()=>{document.getElementById(`create-pa-form`).reset(),document.getElementById(`create-pa-payload-inputs`).innerHTML=``,document.getElementById(`create-pa-modal`).classList.remove(`hidden`)},window.closeCreatePAModal=()=>{document.getElementById(`create-pa-modal`).classList.add(`hidden`)},window.handlePATypeChange=async()=>{let t=document.getElementById(`create-pa-type`).value,n=document.getElementById(`create-pa-payload-inputs`);n.innerHTML=`<p class="text-xs text-slate-400 font-medium">Đang tải dữ liệu cấu hình...</p>`;try{if(t===`promote_user_admin`||t===`demote_admin`){let e=await s(1,1e3,null),r=t===`promote_user_admin`?`user`:`admin`,i=e.filter(e=>e.role===r);if(!i||i.length===0){n.innerHTML=`<p class="text-xs text-rose-500 font-medium">Không tìm thấy ${r} nào để thao tác.</p>`;return}n.innerHTML=`
                <div>
                ...
                <div class="p-5 flex-1 flex flex-col justify-between">
                    ...
                    <div class="mt-4 pt-3.5 border-t border-slate-100 dark:border-slate-800/60">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-xs font-semibold text-emerald-700 dark:text-emerald-400 truncate max-w-[130px]">${z(i)}</span>
                        </div>
                        ...
                        <div class="flex gap-2">
                            <button onclick="showCPDetail('${z(e.id)}')" class="flex-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 py-2.5 rounded-xl text-xs font-bold transition">CHI TIẾT</button>
                            ${e.is_verified?``:`
                                <button onclick="approveCPDirect('${z(e.id)}')" class="bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30 px-3.5 rounded-xl font-bold text-xs transition" title="Duyệt nhanh">✓</button>
                                <button onclick="rejectCPDirect('${z(e.id)}')" class="bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 px-3.5 rounded-xl font-bold text-xs transition" title="Từ chối">✕</button>
                            `}
                        </div>
                    </div>
                </div>
            `,t.appendChild(n)}),J(l||G.length)}catch(e){n.classList.add(`hidden`),t.innerHTML=`<div class="col-span-full p-6 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 text-rose-600 dark:text-rose-400 rounded-xl text-sm font-semibold">Lỗi: ${z(e.message||`Không xác định`)}</div>`}},window.changeCPPage=e=>{window.fetchCollectionPoints(U+e)},window.showCPDetail=e=>{let t=G.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`cp-detail-content`),r=t.contributor_name||(t.created_by?`Cộng đồng`:`Hệ thống`),i=t.created_at?new Date(t.created_at).toLocaleString(`vi-VN`):`N/A`;n.innerHTML=`
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="bg-slate-100 dark:bg-slate-950 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 flex items-center justify-center min-h-[180px]">
                ${t.image_url?`<img src="${z(t.image_url)}" class="w-full h-full object-contain">`:`<div class="p-8 text-center text-slate-400 dark:text-slate-600"><svg class="w-12 h-12 mx-auto mb-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg><p class="font-bold text-xs uppercase tracking-wider">KHÔNG CÓ ẢNH</p></div>`}
            </div>
            <div class="space-y-3">
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Tên điểm thu gom</span>
                    <p class="text-lg font-bold text-slate-900 dark:text-white mt-1 font-heading">${z(t.name||`Chưa đặt tên`)}</p>
                </div>
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Loại điểm</span>
                    <div class="mt-1.5">${q(t.point_type)}</div>
                </div>
                <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Trạng thái</span>
                    <div class="mt-1.5">
        ...
        <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <p class="text-slate-700 dark:text-slate-300 text-sm mt-1 leading-relaxed">${z(t.address||`Chưa có địa chỉ`)}</p>
        </div>
        ${t.description?`
        <div class="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Mô tả</span>
            <p class="text-slate-700 dark:text-slate-300 text-sm mt-1 leading-relaxed">${z(t.description)}</p>
        </div>
        `:``}
        <div class="grid grid-cols-2 gap-3">
            <div class="bg-slate-50 dark:bg-slate-900/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <p class="text-emerald-700 dark:text-emerald-400 font-bold text-xs sm:text-sm mt-0.5 truncate">${z(r)}</p>
            </div>
            <div class="bg-slate-50 dark:bg-slate-900/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
        ...
        ${t.is_verified?``:`
        <div class="pt-4 border-t border-slate-200 dark:border-slate-800 flex gap-3">
            <button onclick="approveCPFromModal('${z(t.id)}')" class="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-xl font-bold text-sm transition shadow-sm">DUYỆT ĐIỂM THU GOM</button>
            <button onclick="rejectCPFromModal('${z(t.id)}')" class="flex-1 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 py-3 rounded-xl font-bold text-sm transition">TỪ CHỐI</button>
        </div>
        `}
    `,document.getElementById(`cp-detail-modal`).classList.remove(`hidden`),document.getElementById(`cp-detail-modal`).classList.add(`flex`)},window.closeCPDetailModal=()=>{document.getElementById(`cp-detail-modal`).classList.add(`hidden`),document.getElementById(`cp-detail-modal`).classList.remove(`flex`)},window.approveCPDirect=async e=>{if(confirm(`Duyệt điểm thu gom này? Sau khi duyệt, điểm sẽ hiển thị trên bản đồ cho tất cả người dùng.`))try{await b(e),alert(`Đã duyệt thành công! Điểm thu gom sẽ hiển thị trên bản đồ.`),window.fetchCollectionPoints(U)}catch(e){alert(`Lỗi duyệt: `+(e.message||`Không xác định`))}},window.rejectCPDirect=async e=>{if(confirm(`Từ chối và xóa điểm thu gom này? Hành động không thể hoàn tác.`))try{await x(e),alert(`Đã từ chối và xóa điểm thu gom.`),window.fetchCollectionPoints(U)}catch(e){alert(`Lỗi: `+(e.message||`Không xác định`))}},window.approveCPFromModal=async e=>{await window.approveCPDirect(e),closeCPDetailModal()},window.rejectCPFromModal=async e=>{await window.rejectCPDirect(e),closeCPDetailModal()};var Y=1,X=12,Z=[];function Q(e){let t=document.getElementById(`btn-q-prev`),n=document.getElementById(`btn-q-next`),r=document.getElementById(`q-page-info`),i=(Y-1)*X+1,a=Math.min(Y*X,e);e===0?(r&&(r.innerText=`Không có dữ liệu`),t&&(t.disabled=!0),n&&(n.disabled=!0)):(r&&(r.innerText=`Hiển thị ${i}-${a} trên ${e}`),t&&(t.disabled=Y===1),n&&(n.disabled=a>=e))}window.changeQPage=e=>{window.fetchGameQuestions(Y+e)},window.fetchGameQuestions=async(e=1)=>{Y=e;let t=document.getElementById(`grid-game-questions`),n=document.getElementById(`loader-game-questions`),r=document.getElementById(`filter-q-status`)?.value;if(!t)return;t.innerHTML=``,n&&n.classList.remove(`hidden`);let i=(Y-1)*X,a=i+X-1;try{let{data:e,count:t}=await S(i,a,r===``?null:r);n&&n.classList.add(`hidden`),Z=e||[],$(Z),Q(t||0)}catch(e){n&&n.classList.add(`hidden`),console.error(`Lỗi tải câu hỏi game:`,e),t.innerHTML=`<div class="col-span-full py-16 text-center text-rose-500 font-bold">Lỗi tải danh sách: ${z(e.message)}</div>`,Q(0)}};function $(e){let t=document.getElementById(`grid-game-questions`);if(t){if(t.innerHTML=``,!e||e.length===0){t.innerHTML=`<div class="col-span-full py-20 text-center text-slate-400 font-bold">Chưa có câu hỏi nào trong hệ thống</div>`;return}e.forEach(e=>{let n=Array.isArray(e.waste_dictionary)?e.waste_dictionary[0]||{}:e.waste_dictionary||{},r=Array.isArray(n.waste_groups)?n.waste_groups[0]||{}:n.waste_groups||{},i=n.name_vi||e.payload?.name_vi||`Chưa đặt tên`,a=r.name_vi||`Chưa phân loại`,o=n.image_url||e.payload?.image_url||``,s=n.fun_fact||e.payload?.fun_fact||``,c=e.is_active,l=e.waste_dictionary_id||n.id||``,u=document.createElement(`div`);u.className=`glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col hover:border-emerald-500/50 transition-all duration-300 shadow-sm`,u.innerHTML=`
            <!-- Card Image (Clickable to Edit) -->
            <div onclick="window.openEditQuestionModal('${z(e.id)}')"
            ...
            <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                <div>
                    <h3 onclick="window.openEditQuestionModal('${z(e.id)}')"
                    ...
                    <div class="min-h-[48px] bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60 mb-4">
                        ...
                        <p class="text-xs text-slate-600 dark:text-slate-300 italic line-clamp-2">
                            ${s?z(s):`<span class="text-slate-400">Không có mẹo kiến thức đi kèm.</span>`}
                        </p>
                    </div>
                </div>
                ...
                <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    <!-- Quick Active Toggle -->
                    <button onclick="window.toggleQuestionActive('${z(e.id)}', ${c})"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="window.openEditQuestionModal('${z(e.id)}')"
                        ...
                        <!-- Delete Button -->
                        <button onclick="window.deleteGameQuestion('${z(e.id)}', '${z(l)}')"
        ...
        `,t.appendChild(u)})}}window.filterQuestionsClient=()=>{let e=(document.getElementById(`filter-q-search`)?.value||``).toLowerCase().trim(),t=document.getElementById(`filter-q-group`)?.value;$(Z.filter(n=>{let r=Array.isArray(n.waste_dictionary)?n.waste_dictionary[0]||{}:n.waste_dictionary||{},i=(r.name_vi||n.payload?.name_vi||``).toLowerCase(),a=String(r.waste_group_id||n.payload?.waste_group_id||``);return(!e||i.includes(e))&&(!t||a===String(t))}))},window.updateQuestionImagePreview=e=>{let t=document.getElementById(`question-preview-box`);if(t){if(!e||!e.trim()){t.innerHTML=`<span class="text-slate-400 text-xs font-medium">Xem trước ảnh sẽ hiển thị ở đây</span>`;return}t.innerHTML=`
        <img src="${z(e.trim())}" alt="Xem trước" class="h-full w-full object-contain p-2"
    ...
    `}},window.openCreateQuestionModal=()=>{let e=document.getElementById(`question-group`);e&&e.options.length<=1&&k&&k.length>0&&(e.innerHTML=`<option value="">-- Chọn nhóm phân loại đúng --</option>`,k.forEach(t=>{e.innerHTML+=`<option value="${t.id}">${t.name_vi}</option>`})),document.getElementById(`question-modal-title`).innerText=`+ Thêm câu hỏi mới vào Game`,document.getElementById(`question-id`).value=``,document.getElementById(`question-dict-id`).value=``,document.getElementById(`question-name`).value=``,e&&(e.value=``),document.getElementById(`question-image-url`).value=``,document.getElementById(`question-funfact`).value=``,document.getElementById(`question-is-active`).checked=!0;let t=document.getElementById(`btn-save-question`);t&&(t.innerText=`LƯU CÂU HỎI VÀO CSDL`),window.updateQuestionImagePreview(``);let n=document.getElementById(`question-modal`);n.classList.remove(`hidden`),n.classList.add(`flex`)},window.openEditQuestionModal=e=>{let t=Z.find(t=>t.id===e);if(!t)return;let n=Array.isArray(t.waste_dictionary)?t.waste_dictionary[0]||{}:t.waste_dictionary||{},r=document.getElementById(`question-group`);r&&r.options.length<=1&&k&&k.length>0&&(r.innerHTML=`<option value="">-- Chọn nhóm phân loại đúng --</option>`,k.forEach(e=>{r.innerHTML+=`<option value="${e.id}">${e.name_vi}</option>`})),document.getElementById(`question-modal-title`).innerText=`✏️ Chỉnh sửa câu hỏi Game`,document.getElementById(`question-id`).value=t.id,document.getElementById(`question-dict-id`).value=t.waste_dictionary_id||n.id||``,document.getElementById(`question-name`).value=n.name_vi||t.payload?.name_vi||``;let i=n.waste_group_id||t.payload?.waste_group_id||``;r&&(r.value=String(i));let a=n.image_url||t.payload?.image_url||``;document.getElementById(`question-image-url`).value=a,document.getElementById(`question-funfact`).value=n.fun_fact||t.payload?.fun_fact||``,document.getElementById(`question-is-active`).checked=!!t.is_active;let o=document.getElementById(`btn-save-question`);o&&(o.innerText=`CẬP NHẬT & LƯU THAY ĐỔI`),window.updateQuestionImagePreview(a);let s=document.getElementById(`question-modal`);s.classList.remove(`hidden`),s.classList.add(`flex`)},window.closeQuestionModal=()=>{let e=document.getElementById(`question-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)},window.submitQuestionForm=async()=>{let t=document.getElementById(`btn-save-question`),n=document.getElementById(`question-id`).value,r=document.getElementById(`question-dict-id`).value,i=document.getElementById(`question-name`).value.trim(),a=document.getElementById(`question-group`).value,o=document.getElementById(`question-image-url`).value.trim(),s=document.getElementById(`question-funfact`).value.trim(),c=document.getElementById(`question-is-active`).checked;if(!i||!a||!o)return alert(`Vui lòng nhập tên vật phẩm, chọn nhóm phân loại và dán URL hình ảnh!`);t.disabled=!0;let l=t.innerText;t.innerText=`ĐANG XỬ LÝ...`;try{if(n)await w({questionId:n,dictId:r,nameVi:i,groupId:a,imageUrl:o,funFact:s,isActive:c}),alert(`Cập nhật câu hỏi thành công!`);else{let t=R(i);t+=`-`+B(4);let{data:{session:n}}=await e.auth.getSession(),r=n?.user?.id;await C({slug:t,nameVi:i,groupId:a,imageUrl:o,funFact:s,isActive:c,userId:r}),alert(`Thêm câu hỏi mới vào Game thành công!`)}window.closeQuestionModal(),window.fetchGameQuestions(Y)}catch(e){console.error(e),alert(`Lỗi lưu câu hỏi: `+(e.message||`Không xác định`))}finally{t.disabled=!1,t.innerText=l}},window.toggleQuestionActive=async(e,t)=>{try{await T(e,!t),window.fetchGameQuestions(Y)}catch(e){alert(`Lỗi đổi trạng thái: `+(e.message||`Không xác định`))}},window.deleteGameQuestion=async(e,t)=>{if(confirm(`Bạn có chắc muốn xóa câu hỏi này khỏi Game? Thao tác này không thể hoàn tác.`))try{await E(e,t),alert(`Đã xóa câu hỏi thành công!`),window.fetchGameQuestions(Y)}catch(e){alert(`Lỗi xóa câu hỏi: `+(e.message||`Không xác định`))}},(async function(){try{let{data:{session:t}}=await e.auth.getSession();if(!t)return window.location.replace(`index.html`);let r=await n(t.user),i=document.getElementById(`admin-email`);i&&(i.innerText=r.email),M=r.role||`admin`,document.getElementById(`auth-loader`).classList.add(`hidden`),window.fetchWasteGroups(),window.fetchSubmissions()}catch(e){alert(`Phiên đăng nhập hết hạn hoặc lỗi: `+e.message),window.location.replace(`index.html`)}})();