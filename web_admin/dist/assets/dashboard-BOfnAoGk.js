import{n as e,r as t,t as n}from"./auth-C0Wlzr71.js";async function r(t){let{data:n,error:r}=await e.from(`waste_submissions`).select(`*`).eq(`status`,t).order(`created_at`,{ascending:!1});if(r)throw r;return n||[]}async function i(t,n){let{data:r,error:i}=await e.from(`waste_submissions`).update({status:n}).eq(`id`,t);if(i)throw i;return r}async function a(){let{data:t,error:n}=await e.from(`waste_groups`).select(`id, name_vi`).order(`sort_order`);if(n)throw n;return t||[]}async function o({slug:t,nameVi:n,funFact:r,imageUrl:i,groupId:a,userId:o}){let{error:s}=await e.from(`waste_dictionary`).insert({slug:t,name_vi:n,fun_fact:r||null,image_url:i||null,waste_group_id:parseInt(a),created_by:o,is_active:!0});if(s)throw s}async function s(t,n,r){let{data:i,error:a}=await e.rpc(`admin_get_users`,{p_page:t,p_limit:n,p_search:r||null});if(a)throw a;return i||[]}async function c(t,n,r){let{error:i}=await e.rpc(`admin_ban_user`,{p_user_id:t,p_reason:r?n:null,p_is_locked:r});if(i)throw i}async function l(t){let{error:n}=await e.auth.resetPasswordForEmail(t);if(n)throw n}async function u(){let{data:t,error:n}=await e.from(`profiles`).select(`id, display_name`);if(n)throw n;return t||[]}async function d(t,n,r){let i=e.from(`privileged_action_requests`).select(`*`,{count:`exact`});t&&(i=i.eq(`state`,t));let{data:a,count:o,error:s}=await i.order(`created_at`,{ascending:!1}).range(n,r);if(s)throw s;return{data:a||[],count:o}}async function f(t){let{data:n,error:r}=await e.from(`privileged_action_approvals`).select(`*`).eq(`request_id`,t);if(r)throw r;return n||[]}async function p(t,n){let{data:r,error:i}=await e.rpc(`privileged_action_add_approval`,{p_request_id:t,p_comment:n||null});if(i)throw i;return r}async function m(t,n){let{error:r}=await e.from(`privileged_action_requests`).update({state:`rejected`,execution_note:n}).eq(`id`,t);if(r)throw r}async function h({actionType:t,payload:n,requesterId:r,reason:i}){let{error:a}=await e.from(`privileged_action_requests`).insert({action_type:t,payload:n,requester_id:r,execution_note:i});if(a)throw a}async function g(t,n){if(t===`promote_user_admin`){let{error:t}=await e.from(`profiles`).update({role:`admin`}).eq(`id`,n.user_id);if(t)throw t}else if(t===`demote_admin`){let{error:t}=await e.from(`profiles`).update({role:`user`}).eq(`id`,n.user_id);if(t)throw t}else if(t===`delete_collection_point`){let{error:t}=await e.from(`collection_points`).delete().eq(`id`,n.point_id);if(t)throw t}else if(t===`toggle_kill_switch`){let{data:t}=await e.from(`system_settings`).select(`value`).eq(`key`,`maintenance`).maybeSingle(),r=t?.value||{enabled:!1,message:``};r.kill_switch=n.kill_switch;let{error:i}=await e.from(`system_settings`).update({value:r}).eq(`key`,`maintenance`);if(i)throw i}else throw Error(`Chưa hỗ trợ thực thi tự động cho loại hành động này.`)}async function _(t,n){let{error:r}=await e.from(`privileged_action_requests`).update({state:`executed`,executed_at:new Date().toISOString(),execution_note:n}).eq(`id`,t);if(r)throw r}async function v(){let{data:t,error:n}=await e.from(`system_settings`).select(`*`);if(n)throw n;return t||[]}async function y({maintenanceVal:t,pointsVal:n,geminiVal:r,userId:i}){let{error:a}=await e.from(`system_settings`).update({value:t,updated_by:i}).eq(`key`,`maintenance`),{error:o}=await e.from(`system_settings`).update({value:n,updated_by:i}).eq(`key`,`points`),{error:s}=await e.from(`system_settings`).update({value:r,updated_by:i}).eq(`key`,`gemini`);if(a||o||s)throw Error(`Lỗi cập nhật cấu hình: `+(a?.message||o?.message||s?.message))}async function b(t,n,r){let i=e.from(`collection_points`).select(`*`,{count:`exact`});t!=null&&t!==``&&(i=i.eq(`is_verified`,t===`true`||t===!0));let{data:a,count:o,error:s}=await i.order(`created_at`,{ascending:!1}).range(n,r);if(s)throw s;return{data:a||[],count:o}}async function x(t){let{error:n}=await e.from(`collection_points`).update({is_verified:!0}).eq(`id`,t);if(n)throw n}async function S(t){let{error:n}=await e.from(`collection_points`).delete().eq(`id`,t);if(n)throw n}async function C(t,n,r){let i=e.from(`game_questions`).select(`
        ...
        `,{count:`exact`});r!=null&&r!==``&&(i=i.eq(`is_active`,r===`true`||r===!0));let{data:a,count:o,error:s}=await i.order(`created_at`,{ascending:!1}).range(t,n);if(s)throw s;return{data:a||[],count:o}}async function w({slug:t,nameVi:n,groupId:r,imageUrl:i,funFact:a,isActive:o,userId:s}){let{data:c,error:l}=await e.from(`waste_dictionary`).insert({slug:t,name_vi:n,waste_group_id:r,image_url:i,fun_fact:a,created_by:s,is_active:!0}).select(`id`).single();if(l)throw l;let{error:u}=await e.from(`game_questions`).insert({waste_dictionary_id:c.id,game_types:[`quiz`],payload:{image_url:i,fun_fact:a},is_active:o});if(u)throw u}async function T({questionId:t,dictId:n,nameVi:r,groupId:i,imageUrl:a,funFact:o,isActive:s}){if(n){let{error:t}=await e.from(`waste_dictionary`).update({name_vi:r,waste_group_id:i,image_url:a,fun_fact:o,updated_at:new Date().toISOString()}).eq(`id`,n);if(t)throw t}let{error:c}=await e.from(`game_questions`).update({is_active:s,payload:{image_url:a,fun_fact:o},updated_at:new Date().toISOString()}).eq(`id`,t);if(c)throw c}async function E(t,n){let{error:r}=await e.from(`game_questions`).update({is_active:n,updated_at:new Date().toISOString()}).eq(`id`,t);if(r)throw r}async function ee(t,n){let{error:r}=await e.from(`game_questions`).delete().eq(`id`,t);if(r)throw r;n&&await e.from(`waste_dictionary`).delete().eq(`id`,n).catch(()=>{})}window.db=e,window.handleLogout=t,window.toggleMobileSidebar=e=>{let t=document.getElementById(`sidebar`),n=document.getElementById(`sidebar-backdrop`);if(!t)return;let r=t.classList.contains(`-translate-x-full`);(typeof e==`boolean`?e:r)?(t.classList.remove(`-translate-x-full`),n?.classList.remove(`hidden`)):(t.classList.add(`-translate-x-full`),n?.classList.add(`hidden`))},window.toggleTheme=()=>{document.documentElement.classList.contains(`dark`)?(document.documentElement.classList.remove(`dark`),localStorage.setItem(`ecosort_theme`,`light`)):(document.documentElement.classList.add(`dark`),localStorage.setItem(`ecosort_theme`,`dark`)),D()};function D(){let e=document.documentElement.classList.contains(`dark`),t=document.getElementById(`theme-toggle-btn`);t&&(t.innerHTML=e?`
    ...
    `,document.getElementById(`cp-detail-modal`).classList.remove(`hidden`),document.getElementById(`cp-detail-modal`).classList.add(`flex`)},window.closeCPDetailModal=()=>{document.getElementById(`cp-detail-modal`).classList.add(`hidden`),document.getElementById(`cp-detail-modal`).classList.remove(`flex`)},window.approveCPDirect=async e=>{if(confirm(`Duyệt điểm thu gom này? Sau khi duyệt, điểm sẽ hiển thị trên bản đồ cho tất cả người dùng.`))try{await x(e),alert(`Đã duyệt thành công! Điểm thu gom sẽ hiển thị trên bản đồ.`),window.fetchCollectionPoints(U)}catch(e){alert(`Lỗi duyệt: `+(e.message||`Không xác định`))}},window.rejectCPDirect=async e=>{if(confirm(`Từ chối và xóa điểm thu gom này? Hành động không thể hoàn tác.`))try{await S(e),alert(`Đã từ chối và xóa điểm thu gom.`),window.fetchCollectionPoints(U)}catch(e){alert(`Lỗi: `+(e.message||`Không xác định`))}},window.approveCPFromModal=async e=>{await window.approveCPDirect(e),closeCPDetailModal()},window.rejectCPFromModal=async e=>{await window.rejectCPDirect(e),closeCPDetailModal()};var Y=1,X=12,Z=[];function Q(e){let t=document.getElementById(`btn-q-prev`),n=document.getElementById(`btn-q-next`),r=document.getElementById(`q-page-info`),i=(Y-1)*X+1,a=Math.min(Y*X,e);e===0?(r&&(r.innerText=`Không có dữ liệu`),t&&(t.disabled=!0),n&&(n.disabled=!0)):(r&&(r.innerText=`Hiển thị ${i}-${a} trên ${e}`),t&&(t.disabled=Y===1),n&&(n.disabled=a>=e))}window.changeQPage=e=>{window.fetchGameQuestions(Y+e)},window.fetchGameQuestions=async(e=1)=>{Y=e;let t=document.getElementById(`grid-game-questions`),n=document.getElementById(`loader-game-questions`),r=document.getElementById(`filter-q-status`)?.value;if(!t)return;t.innerHTML=``,n&&n.classList.remove(`hidden`);let i=(Y-1)*X,a=i+X-1;try{let{data:e,count:t}=await C(i,a,r===``?null:r);n&&n.classList.add(`hidden`),Z=e||[],$(Z),Q(t||0)}catch(e){n&&n.classList.add(`hidden`),console.error(`Lỗi tải câu hỏi game:`,e),t.innerHTML=`<div class="col-span-full py-16 text-center text-rose-500 font-bold">Lỗi tải danh sách: ${z(e.message)}</div>`,Q(0)}};function $(e){let t=document.getElementById(`grid-game-questions`);if(t){if(t.innerHTML=``,!e||e.length===0){t.innerHTML=`<div class="col-span-full py-20 text-center text-slate-400 font-bold">Chưa có câu hỏi nào trong hệ thống</div>`;return}e.forEach(e=>{let n=e.waste_dictionary||{},r=n.waste_groups||{},i=n.name_vi||`Chưa đặt tên`,a=r.name_vi||`Chưa phân loại`,o=n.image_url||e.payload?.image_url||``,s=n.fun_fact||e.payload?.fun_fact||``,c=e.is_active,l=e.waste_dictionary_id||n.id||``,u=document.createElement(`div`);u.className=`glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col hover:border-emerald-500/50 transition-all duration-300 shadow-sm`,u.innerHTML=`
            ...
            <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                <div>
                    <h3 class="text-base font-bold text-slate-900 dark:text-white line-clamp-1 font-heading mb-1.5" title="${z(i)}">
                        ${z(i)}
                    </h3>
        ...
        `,t.appendChild(u)})}}window.filterQuestionsClient=()=>{let e=(document.getElementById(`filter-q-search`)?.value||``).toLowerCase().trim(),t=document.getElementById(`filter-q-group`)?.value;$(Z.filter(n=>{let r=n.waste_dictionary||{},i=(r.name_vi||``).toLowerCase(),a=r.waste_group_id||``;return(!e||i.includes(e))&&(!t||a===t)}))},window.updateQuestionImagePreview=e=>{let t=document.getElementById(`question-preview-box`);if(t){if(!e||!e.trim()){t.innerHTML=`<span class="text-slate-400 text-xs font-medium">Xem trước ảnh sẽ hiển thị ở đây</span>`;return}t.innerHTML=`
    ...
    `}},window.openCreateQuestionModal=()=>{document.getElementById(`question-modal-title`).innerText=`Thêm câu hỏi vào Game`,document.getElementById(`question-id`).value=``,document.getElementById(`question-dict-id`).value=``,document.getElementById(`question-name`).value=``,document.getElementById(`question-group`).value=``,document.getElementById(`question-image-url`).value=``,document.getElementById(`question-funfact`).value=``,document.getElementById(`question-is-active`).checked=!0,document.getElementById(`btn-save-question`).innerText=`LƯU CÂU HỎI VÀO CSDL`,window.updateQuestionImagePreview(``);let e=document.getElementById(`question-modal`);e.classList.remove(`hidden`),e.classList.add(`flex`)},window.openEditQuestionModal=e=>{let t=Z.find(t=>t.id===e);if(!t)return;let n=t.waste_dictionary||{};document.getElementById(`question-modal-title`).innerText=`Chỉnh sửa câu hỏi Game`,document.getElementById(`question-id`).value=t.id,document.getElementById(`question-dict-id`).value=t.waste_dictionary_id||n.id||``,document.getElementById(`question-name`).value=n.name_vi||``,document.getElementById(`question-group`).value=n.waste_group_id||``;let r=n.image_url||t.payload?.image_url||``;document.getElementById(`question-image-url`).value=r,document.getElementById(`question-funfact`).value=n.fun_fact||t.payload?.fun_fact||``,document.getElementById(`question-is-active`).checked=!!t.is_active,document.getElementById(`btn-save-question`).innerText=`CẬP NHẬT CÂU HỎI`,window.updateQuestionImagePreview(r);let i=document.getElementById(`question-modal`);i.classList.remove(`hidden`),i.classList.add(`flex`)},window.closeQuestionModal=()=>{let e=document.getElementById(`question-modal`);e.classList.add(`hidden`),e.classList.remove(`flex`)},window.submitQuestionForm=async()=>{let t=document.getElementById(`btn-save-question`),n=document.getElementById(`question-id`).value,r=document.getElementById(`question-dict-id`).value,i=document.getElementById(`question-name`).value.trim(),a=document.getElementById(`question-group`).value,o=document.getElementById(`question-image-url`).value.trim(),s=document.getElementById(`question-funfact`).value.trim(),c=document.getElementById(`question-is-active`).checked;if(!i||!a||!o)return alert(`Vui lòng nhập tên vật phẩm, chọn nhóm phân loại và dán URL hình ảnh!`);t.disabled=!0;let l=t.innerText;t.innerText=`ĐANG XỬ LÝ...`;try{if(n)await T({questionId:n,dictId:r,nameVi:i,groupId:a,imageUrl:o,funFact:s,isActive:c}),alert(`Cập nhật câu hỏi thành công!`);else{let t=R(i);t+=`-`+B(4);let{data:{session:n}}=await e.auth.getSession(),r=n?.user?.id;await w({slug:t,nameVi:i,groupId:a,imageUrl:o,funFact:s,isActive:c,userId:r}),alert(`Thêm câu hỏi mới vào Game thành công!`)}window.closeQuestionModal(),window.fetchGameQuestions(Y)}catch(e){console.error(e),alert(`Lỗi lưu câu hỏi: `+(e.message||`Không xác định`))}finally{t.disabled=!1,t.innerText=l}},window.toggleQuestionActive=async(e,t)=>{try{await E(e,!t),window.fetchGameQuestions(Y)}catch(e){alert(`Lỗi đổi trạng thái: `+(e.message||`Không xác định`))}},window.deleteGameQuestion=async(e,t)=>{if(confirm(`Bạn có chắc muốn xóa câu hỏi này khỏi Game? Thao tác này không thể hoàn tác.`))try{await ee(e,t),alert(`Đã xóa câu hỏi thành công!`),window.fetchGameQuestions(Y)}catch(e){alert(`Lỗi xóa câu hỏi: `+(e.message||`Không xác định`))}},(async function(){try{let{data:{session:t}}=await e.auth.getSession();if(!t)return window.location.replace(`index.html`);let r=await n(t.user),i=document.getElementById(`admin-email`);i&&(i.innerText=r.email),M=r.role||`admin`,document.getElementById(`auth-loader`).classList.add(`hidden`),window.fetchWasteGroups(),window.fetchSubmissions()}catch(e){alert(`Phiên đăng nhập hết hạn hoặc lỗi: `+e.message),window.location.replace(`index.html`)}})();